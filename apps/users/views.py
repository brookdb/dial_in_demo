...
from apps.brew.models import Brew
...
def dashboard_view(request, user_id):
    if user_id == str(request.user.id):
        ...
        user_brews = Brew.objects.filter(userID=request.user).order_by('-brewDate')
        return render(request, 'users/dashboard.html', {'user': user, 'brews': user_brews})
    else:
        return redirect('apps.users:login')
